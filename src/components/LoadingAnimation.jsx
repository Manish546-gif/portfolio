...
export default function LoadingScreen({ onComplete, shouldClose = false }) {
  ...
  // Also auto-close after 100% if shouldClose doesn't trigger within 1.5s
  useEffect(() => {
  ...
  useEffect(() => {
    if (pct === 100 && !shouldClose) {
      // Auto-close after 1.5 seconds if image hasn't loaded
    ...
    <AnimatePresence onExitComplete={handleExitComplete}>
      {!done && (
        <motion.div
          ...
          transition={{ duration: 1.5, ease: DARK }}
        >
          {/* vertical edge lines */}
          {['left', 'right'].map(side => (
            <motion.div
              ...
              initial={{ scaleY: 0 }}
              animate={{ scaleY: 1 }}
              transition={{ delay: 0, duration: 0.9, ease: EXPO }}
            />
          ))}

          {/* corners */}
          <motion.span style={{ ...CORNER, top: '32px', left: '30px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0, duration: 0.5 }}>Manish Kumar</motion.span>
          <motion.span style={{ ...CORNER, top: '32px', right: '40px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0, duration: 0.5 }}>2025</motion.span>
          <motion.span style={{ ...CORNER, bottom: '38px', left: '40px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0, duration: 0.5 }}>Portfolio</motion.span>
